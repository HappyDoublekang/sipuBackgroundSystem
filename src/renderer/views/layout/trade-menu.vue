	...
	<el-aside width="190px" style="background: #FFFFFF;box-shadow: 0 1px 5px 0 rgba(0,0,0,0.10);">
	  <el-menu theme="dark" :default-active="activeIndex" router @select="handleSelect" :default-openeds="['1']" style="border:none">
	    ...
	    <el-submenu index="2" v-if="$store.getters.hasPermission('orderManageRead')">
	      <template slot="title"><i class="el-icon-document-add"></i>订单</template>
	      ...
	      <el-menu-item index="order-advance" data-title="订单">预订单管理</el-menu-item>
	      <el-menu-item index="order-produce" data-title="订单">生产单管理</el-menu-item>
	      <el-menu-item index="order-invoice" data-title="订单">申请开票</el-menu-item>
	      <el-menu-item index="order-query" data-title="订单">订单查询</el-menu-item>
	      <el-menu-item index="order-original-change" data-title="订单">原始订单管理</el-menu-item>
	    </el-submenu>
	    <el-submenu index="3" v-if="$store.getters.hasPermission('deliverRead')">
	      <template slot="title"><i class="el-icon-message"></i>发货</template>
	      <el-menu-item index="shipping-management" data-title="发货">发货管理</el-menu-item>
	    </el-submenu>
	    <el-submenu index="4" v-if="$store.getters.hasPermission('afterSalesRead')">
	      <template slot="title"><i class="el-icon-message"></i>售后</template>
				<el-menu-item index="returnOrder-management" data-title="售后">退货单管理</el-menu-item>
	      <el-menu-item index="repairOrder-management" data-title="售后">维修单管理</el-menu-item>
	      <el-menu-item index="reissueGood-management" data-title="售后">补发货物管理</el-menu-item>
	      <el-menu-item index="feedback-management" data-title="售后">反馈管理</el-menu-item>
				<el-menu-item index="accountability-management" data-title="售后">追责管理</el-menu-item>
	      <el-menu-item index="rebate-management" data-title="售后">反款管理</el-menu-item>
	    </el-submenu>
	    <el-submenu index="5" v-if="$store.getters.hasPermission('purchaseRead')">
	      ...
	      <el-menu-item index="suppliers-goods" v-if="$store.getters.hasPermission('supplierGoodsRead')" data-title="采购">供应商货品</el-menu-item> -->
				<el-menu-item index="procurementApply-management" data-title="采购">采购申请管理</el-menu-item>
	      <el-menu-item index="procurementOrder-management" data-title="采购">采购单管理</el-menu-item>
	    </el-submenu>
	    <el-submenu index="6" v-if="$store.getters.hasPermission('stockRead')">
	      <template slot="title"><i class="el-icon-goods"></i>仓库</template>
	      <!-- <el-menu-item class="el-menu-item" @click="$store.commit('setWarehouseManagementVisible', true)" v-if="$store.getters.hasPermission('warehouseManageRead')" data-title="仓库">仓库管理</el-menu-item> -->
	      ...
	      <!-- <el-menu-item index="product-order-management" v-if="$store.getters.hasPermission('productOrderManageRead')" data-title="仓库">生产单管理</el-menu-item> -->
	    	<el-menu-item index="outgoing-management" v-if="$store.getters.hasPermission('outgoingManageRead')" data-title="仓库">出库管理</el-menu-item>
				<el-menu-item index="warehousing-management" v-if="$store.getters.hasPermission('warehousingManageRead')" data-title="仓库">入库管理</el-menu-item>
				<el-menu-item index="inventory-management" v-if="$store.getters.hasPermission('inventoryManageRead')" data-title="仓库">库存管理</el-menu-item>
				<el-menu-item index="bulk-purchase-warehousing" v-if="$store.getters.hasPermission('bulkPurchaseWarehousingRead')" data-title="仓库">批量采购入库</el-menu-item>
			</el-submenu>
  ...
  export default {
    ...
    methods: {
      ...
      handleSelect(key, keyPath, item) {
		...
		// console.log('打印数据' + JSON.stringify(tabItem1))
      }
	},
	mounted() {
		this.handleSelect()
